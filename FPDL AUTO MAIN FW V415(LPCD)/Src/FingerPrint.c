...
void FPMcmd_Excute(void)
{
	...
	{
		if ( FpmAckMgr.TimeOutCnt > 1 )
		{
			FpmAckMgr.TimeOutCnt--;
		}
		else if ( FpmAckMgr.TimeOutCnt == 1 )
		{
			SET_FPMPOWER_OFF;
	...
	CmdLenth = (UART1_Mgr.RX_Buffer[7]*256) + UART1_Mgr.RX_Buffer[8] + 9;		
	for (i=0;i<CmdLenth;i++)
	{
		FpmAckMgr.Buff[i] = UART1_Mgr.RX_Buffer[i];		
	}
