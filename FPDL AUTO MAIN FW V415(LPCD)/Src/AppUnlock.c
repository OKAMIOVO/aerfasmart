...
u_int time_findyear(u_int year/*2019*/,u_int days/*1-366*/,u_int *pmonth,u_int *pday)
{
	...
	if( days>maxdays )
	{
		//debug_print("findyear year=%d %d>%d\n",year,days,maxdays);
		return maxdays;
	}
	for(i=0;i<12;i++)
    {
    ...
    //debug_print("findyear year=%d.%02d.%02d  days=%d\n",year,*pmonth,*pday,days);
    return days;
}
...
u_int time_adays(itime *it)
{
	...
	//debug_print("day_offset_from_baseyear  days=%d d1=%d d2=%d\n",days,days/6,days%6);
    return days;
}
...
s_int  time_diff_day(itime t1,itime t2)
{
	...
	//debug_print( " time_diff_day a1=%d a2=%d diff=%d\n",t1.aday,t2.aday,diff);
	return diff;
}
...
s_int  time_diff_minute(itime t1,itime t2)
{
	...
	//debug_print( " time_diff_minute a1=%d a2=%d diff=%d\n",t1.aday,t2.aday,diff_minute);
	return diff_minute;
}
...
bool pwd_userpwd_select_d(ipwd *pwd,u_char userpwd[],u_int userpwdlen)
{
	...
	u_int pwdlen = userpwdlen;
	u_int data = hash_user[(userpwdlen-6)%5][pwd->rand]; ////??????
	u_int d1 = data/100;
	u_int d2 = (data%100)/10;
	u_int d3 = data%10;
	u_int d4 = 0;
	u_int dc = 0;
	
	u_int bten = userpwdlen-11;
	...
	//debug_print( "pwd_userpwd_select_d rand=%d data=%d index=%d bten=%d buf=%s ret=%d\n", pwd->rand,data,(pwdlen-6)%5,bten,pwd->userpwd_debug,pwd->userpwd_iscorrect);
...
{
	u_int data = 0;
	u_char d1 = (u_char)(data/10000);
	u_char d2 = (u_char)((data%10000)/1000);
	u_char d3 = (u_char)((data%1000)/100);
	u_char d4 = (u_char)((data%100)/10);
	u_char d5 = (u_char)(data%10);
	u_char d6 = 0;
	u_char i=0;
	
	if( pwd==NULL )
		return false;
	data = hash_6bit[pwd->rand];
...
unsigned int pwd_decrypt_checkmode(u_char userpwd[],u_int userpwdlen,u_char lockpwd[],u_int lockpwd_len)
{
	ipwd *pwdtmp=pwd_decrypt(userpwd,userpwdlen,lockpwd,lockpwd_len);
	if( pwdtmp!=NULL )
	{
...
void AppUnlockTest(void)
{
    ...
    unsigned char lockpwd[12]={2,0,2,2,9,3,6,9,0,9,5,4};
  	unsigned char userpwd[12]={3,4,5,6,4,5,3,5};  //mode =3, 00:00~11:00 
        ...
        //userpwd是门锁上设定的管理员密码；
	test_result = pwd_decrypt_checkmode(userpwd,8,lockpwd,12);   //解码开锁密码lockpwd，传入整形数组的用户密码、用户密码长度、整形数组的开锁密码
